...
#+TODO: TRAIL_RUN(t) REFACTOR_NEEDED(r) BROKEN(b) |
...
** BROKEN Pre-Commit
NOTE: This is currently broken see [[*Status][Status]] for more.
...
*** Status
This is currently broken when used with recent versions of =transient=. Attempting to invoke =magit= results in the following error message

#+begin_quote
=transient--parse-suffix: Need command, argument, ':info', ':info*' or ':cons'; got ':description'=
#+end_quote

Github user [[https://github.com/glucas][glucas]] as reported this in ticket [[https://github.com/DamianB-BitFlipper/magit-pre-commit.el/issues/3][DamianB-BitFlipper/magit-pre-commit.el #3]], last updated Jan 25, 2026. This should be a simple fix, but I don't presently have the time to implement it myself. I've disabled the package for now, but will re-enable it once this is resolved.
...
First lets setup the dependency. I've pinned this to the version 3.0 release. 4.0 introduced chat function that conflicts with functions from [[*CoPilot Chat][CoPilot Chat]]. I could ignore not load the .el function with the conflicts, but eh.
#+begin_src emacs-lisp :tangle "modules/tools/llm/packages.el" :noweb no-export :comments no
(package! copilot
  :recipe (:host github :repo "copilot-emacs/copilot.el" :files ("*.el"))
  :pin "b98754712ad05282e1bb392bb35e5918ed35c9b6"
  )
#+end_src
...
** Agent Mode
There's a good number of agent style tools. I've previously tried [[https://eca.dev/][eca]] (Editor Code Assistant), but bounced off it. I'm instead I'm going to try [[https://xenodium.com/][Xenodium]]'s [[https://github.com/xenodium/agent-shell][agent-shell]].

Lets install the requirements first. Dependencies are listed on the [[https://github.com/xenodium/agent-shell?tab=readme-ov-file#doom-emacs][GitHub readme]].
...
Next lets setup the packages with =use-package!=.
...
Since I'm currently using GitHub's CoPilot for my LLM needs, the copilot-cli will be needed as well. [[https://docs.github.com/en/copilot/concepts/agents/about-copilot-cli][See here for more]]. Install of this package will be left to the user.
